...
- **`src/models/baseModel.ts`** — abstract `BaseModel<T>` (Template Method). Validates the table name and every column name via `assertValidIdentifier`, resolves the primary key (from `primaryKey: true`, or a backend default like Mongo's `_id`), and offers `assertKnownColumns()` to reject payload/criteria keys outside the schema. Declares the abstract CRUD signatures.
...
- **Identifiers are only ever interpolated after `assertValidIdentifier`;** values always go through parameterized queries / driver filters. Never build SQL by concatenating unvalidated names or values.
